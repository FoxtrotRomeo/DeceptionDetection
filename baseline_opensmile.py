...
for filename in all_files:
    ...
    # delete the .csv extension
    name = name[:-4]
    # read the file into a dataframe. Use the first column as index
    df = pd.read_csv(filename, index_col='Unnamed: 0', header=0)
    # drop the columns starting with timestamp
    df = df.drop(df.filter(regex='timestamp').columns, axis=1)
    # store the dataframe in the dictionary
...
with open('results.txt', 'a') as f:
    f.write('Rocket on merged_opensmile_out\n')
    ...
    f.write(cr_rocket)
    f.write('\n')
    f.write('Rocket on opensmile_out_A\n')
    f.write('Confusion Matrix:\n')
    ...
    f.write(cr_rocket_A)
    f.write('\n')
    f.write('CIF on merged_opensmile_out\n')
    f.write('Confusion Matrix:\n')
    f.write(str(cm_cif))
    f.write('\n')
    f.write('Classification Report:\n')
    f.write(cr_cif)
    f.write('\n')
    f.write('CIF on opensmile_out_A\n')
    f.write('Confusion Matrix:\n')
